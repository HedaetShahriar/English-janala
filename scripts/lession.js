...
const modal=(vocabulary)=>{
    ...
    div.classList.add('border-slate-200', 'border', 'text-left', 'p-3','md:p-6' , 'rounded-xl', 'font-semibold','text-xl' , 'md:text-2xl');
    div.innerHTML=`
        <h1 class="text-2xl md:text-3xl lg:text-4xl mb-4 md:mb-6 lg:mb-8">
            ${vocabulary.word} (
            <i onclick="pronounceWord('${vocabulary.word}')" class="fa-solid fa-microphone-lines cursor-pointer">
            </i>: 
            ${vocabulary.pronunciation})
        </h1>

        <h2 class="md:mb-2.5">Meaning</h2>
        <h2 class="font-medium bangla">${vocabulary.meaning || 'unavailable'}</h2>
    ...
    synonymsDiv.classList.add('flex', 'gap-2', 'md:mt-2.5', 'font-normal', 'text-xl');
...
const showLessons=(lessons)=>{
    ...
    if (!lessons || lessons.length === 0) {
        ...
        lessonDiv.classList.add('flex', 'flex-col', 'items-center', 'justify-center', 'py-12', 'col-span-3', 'text-center');
        lessonDiv.innerHTML = `
            ...
            <div class="py-8 md:py-10 lg:py-12 px-4 md:px-6 drop-shadow-sm inter bg-white rounded-xl">
                <div class="md:h-[150px] lg:h-[160px]">
                    ...
                    <h1 class="text-3xl bangla font-semibold">
                        "${lesson.meaning || 'unavailable'}/ ${lesson.pronunciation}"
                    </h1>
                </div>
